...
{
    /* 0x00 */ float spawnLocation[3];
    /* 0x0C */ float directionAngle;
    /* 0x10 */ int flag;
    /* 0x14 */ int heartCount;
    /* 0x18 */ int u1;
    /* 0x1A */ //short bombType;
    /* 0x1C */ int bombCount;
    /* 0x24 */ int bombExplosionLevel;
    /* 0x24 */ int controlType;              // 0 = None, 1 = Player, 2 = AI, 3 = Dead (No Input)
    /* 0x28 */ LevelClass *PlayerLevelClass; // Pointer to the LevelClass this Player is controlling
    /* 0x2C */ LevelClass *HeldLevelClass;   // Pointer to the LevelClass of the held object
    /* 0x30 */ LevelClass *OtherClass;
    /* 0x34 */ int u8;
    /* 0x38 */ int u9;
    /* 0x3C */ int u10;
    /* 0x40 */ int u11;
    /* 0x44 */ int animationType;
    /* 0x48 */ int collisionEnabled;
    /* 0x4C */ int u13;
    /* 0x50 */ int u14;
    /* 0x54 */ int u15;
    /* 0x58 */ float directionAngle2[3];
    /* 0x64 */ int unk64;
    /* 0x68 */ int u12;
    /* 0x6A */ //char timerA_Enabled; // virusType has to be set for this to work
    /* 0x6B */ char virusType;
    /* 0x6C */ float virusFloat1;
    /* 0x70 */ int virusFloat2;
    /* 0x74 */ int timer2;
    /* 0x78 */ int virusTimer;
    /* 0x7C */ int u16[5];
    /* 0x90 */ int timerA;
    ...
    /* 0x98 */ int unkType2;
    /* 0x9C */ int u17[4];
    /* 0xB0 */ LevelClass *ComputationLevelClass;
    /* 0xB4 */ int u20[3];
    /* 0xC0 */ float unknownFloat1;
    /* 0xC4 */ int u21;
} Player; // size:0xC8

extern Player g_Players[];
extern s32 D_802AC8A8[];
extern s32 D_802AC8E8[];
extern f32 D_802ACA78[];

extern f32 D_802A3164;

typedef struct ComputerPlayerMovement {
	/* 0x00 */ float moveDirection;
	/* 0x04 */ float f1;
	/* 0x08 */ float moveSpeed;
	/* 0x0C */ short u1;
	/* 0x0E */ short u2;
} ComputerPlayerMovement; // size:0x10

extern ComputerPlayerMovement g_ComputerPlayerMovement[];

void* g_InitLevelClass(s32, s32, s32, s32);                    /* extern */
LevelClass *g_SpawnAndAttachRoundShadow(void*);                           /* extern */
s32 g_AssignModelParts(void*, s32, s32, s32);              /* extern */
int g_InitPlayerAssetPointers(Object *playerObject, int count); //0x802280F4
void g_LoadCustomParts(int playerID, int partTypeID, int partSetID); //0x8023A734
...
extern CustomParts g_SaveGameCustomParts; //8008E5A8 Not working. Have to index from D_8008E570[], which is dumb.
extern int D_8008E570[];

enum CustomBombermanMode {
    On,
    Off,
    OptionCount
};

RECOMP_PATCH void g_SpawnPlayer(int id) {
    s32 i;
    s32 var_s1;
    s32 var_v1;
    ...
    levelClass = g_InitLevelClass(0x49, 0x0D000011, 0x70, 0);
    player = &g_Players[id];
    player->PlayerLevelClass = levelClass;
    levelClass->ObjectPointer->position[0] = player->spawnLocation[0];
    player->PlayerLevelClass->ObjectPointer->position[1] = player->spawnLocation[1];
    player->PlayerLevelClass->ObjectPointer->position[2] = player->spawnLocation[2];
    player->PlayerLevelClass->ObjectPointer->angle[1] = player->directionAngle;
    player->PlayerLevelClass->u11 = 1;
    player->PlayerLevelClass->radius = 30.0f;
    player->PlayerLevelClass->unk_float = 51.0f;
    player->PlayerLevelClass->unk_float2 = 0.0f;
    player->PlayerLevelClass->unk_float3 = D_802A3164;
    player->PlayerLevelClass->unk_float1 = 0.0f;
    player->PlayerLevelClass->u2 = 0x1FF0;
    func_80229F74(player->PlayerLevelClass->ObjectPointer, 2);
    func_8022984C(player->PlayerLevelClass->ObjectPointer, 0xC);
    var_v1 = 0;
    if ((player->u12 & 0x200) == 0x200) {
        var_v1 = 1;
    }
    if (var_v1 != 0) {
        LevelClass *levelClass = player->PlayerLevelClass;
        func_8022979C(levelClass->ObjectPointer, 4, 4);
        func_8022979C(levelClass->ObjectPointer, 5, 5);
        func_80231408(levelClass, 0x80000);
        levelClass->ObjectPointer->u7 = 0x80;
        levelClass->ObjectPointer->param |= 0x1000;
    }

    for(i = 0; i < 0xC; i++) {
        switch (id) {                             
            case 0: var_s1 = 0x4A; break;
            case 1: var_s1 = 0x4B; break;
            case 2: var_s1 = 0x4C; break;
            case 3: var_s1 = 0x4D; break;
        }
        g_AssignModelParts(player->PlayerLevelClass->ObjectPointer, var_s1, i, i);
    }
    player->ComputationLevelClass = g_SpawnAndAttachRoundShadow(player->PlayerLevelClass);
    D_802AC8A8[id] = 1;
    D_802AC89C = 0;
    D_802AC8A0 = 0;
    func_802444B8(id, 0);
    ...
    player->u9 = 0;
    player->u1 &= ~0x404;
    player->virusFloat2 = 0;
    player->u11 = 0;
    g_ComputerPlayerMovement[id].u1 = 0;
    g_ComputerPlayerMovement[id].u2 = 0;
    g_ComputerPlayerMovement[id].moveSpeed = 0.0f;
    g_ComputerPlayerMovement[id].f1 = 0.0f;
    g_ComputerPlayerMovement[id].moveDirection = 0.0f;

    // check for option if toggled
    if (recomp_get_custom_bomberman_mode() && (id == 0 && !(g_LevelLoader.flag & 0x4))) {
        LevelClass *levelClass = g_Players[id].PlayerLevelClass;

        if (levelClass) {
            Object *obj = levelClass->ObjectPointer;

            if (obj) {
                g_InitPlayerAssetPointers(obj, 0xF);

                g_LoadCustomParts(id, 0, D_8008E570[14]); // g_SaveGameCustomParts.customHeadID
                g_LoadCustomParts(id, 1, D_8008E570[15]); // g_SaveGameCustomParts.customBodyID
                g_LoadCustomParts(id, 2, D_8008E570[16]); // g_SaveGameCustomParts.customArmsID
                g_LoadCustomParts(id, 3, D_8008E570[17]); // g_SaveGameCustomParts.customLegsID
            }
        }
    }
}
